...
#ifdef __APPLE__
class CrealityPrintTaskBarIcon : public wxTaskBarIcon
{
public:
    CrealityPrintTaskBarIcon(wxTaskBarIconType iconType = wxTBI_DEFAULT_TYPE) : wxTaskBarIcon(iconType) {}
...
*/
    ...
    case GUI_App::EAppMode::Editor:
        m_taskbar_icon = std::make_unique<CrealityPrintTaskBarIcon>(wxTBI_DOCK);
        m_taskbar_icon->SetIcon(wxIcon(Slic3r::var("CrealityPrint-mac_256px.ico"), wxBITMAP_TYPE_ICO), "CrealityPrint");
        break;
...
void MainFrame::print_plate(PrintSelectType tp){
    ...
    else if (m_print_select == ePrintPlate)
    {
        ...
        if(is_cx_vendor){
            this->select_tab((int)MainFrame::tpDeviceMgr);
        }
        else
        {
            wxPostEvent(m_plater, SimpleEvent(EVT_GLTOOLBAR_PRINT_PLATE));
...
bool MainFrame::get_enable_print_status(bool is_all_or_any_of_them)
{
    bool enable = true;
    ...
    else if (m_print_select == eSendToLocalNetPrinter || m_print_select == eUploadGcode)
    {
        ...
        if(enable && is_all_plates)
        {
            PartPlate* firstPartPlate = part_plate_list.get_plate(0);
            if(firstPartPlate)
            {
                if(!firstPartPlate->is_slice_result_ready_for_print())
                {
                    enable = false;
                }
            }
    ...
    else if (m_print_select == eExportAllSlicedFile)
    {
        ...
        {
            if (!part_plate_list.is_are_the_slice_results_ready_to_export())
            {
                enable = false;
            }
        }
...
static wxMenu* generate_help_menu()
{
    ...
    // Report a bug
    //append_menu_item(helpMenu, wxID_ANY, _L("Report Bug(TODO)"), _L("Report a bug of CrealityPrint"),
    //    [](wxCommandEvent&) {
    //        //TODO
    //    });
    // Check New Version
...
//#endif
        ...
        append_menu_item(
            import_menu, wxID_ANY, _L("Import 5.x Presets") + dots /*+ "\tCtrl+I"*/, _L("Import 5.x Presets"),
            [this](wxCommandEvent&) { loadOldPresetsFromFolder(); }, "menu_import", nullptr, [this]() { return true; }, this);
        ...
        append_menu_item(
            ...
            [this](wxCommandEvent&) { export_presets(); }, "menu_export_config", nullptr, []() { return true; }, this);
    ...
    if (m_plater) {
        ...
        viewMenu->AppendSeparator();
        append_menu_check_item(viewMenu, wxID_ANY, _L("Show &Wireframe"), _L("Show wireframes in 3D scene"),
            [this](wxCommandEvent&) { m_plater->toggle_show_wireframe(); m_plater->get_current_canvas3D()->post_event(SimpleEvent(wxEVT_PAINT)); }, this,
            [this]() { return m_plater->is_wireframe_enabled()&&plater()->is_view3D_shown(); }, [this]() { return m_plater->is_show_wireframe(); }, this);
    ...
    //auto preference_item = new wxMenuItem(parent_menu, CrealityPrintMenuPreferences + bambu_studio_id_base, _L("Preferences") + "\tCtrl+,", "");
#else
...
//    });
    ...
    //auto about_item = new wxMenuItem(parent_menu, CrealityPrintMenuAbout + bambu_studio_id_base, about_title, "");
        //parent_menu->Bind(wxEVT_MENU, [this, bambu_studio_id_base](wxEvent& event) {
        //    switch (event.GetId() - bambu_studio_id_base) {
        //        case CrealityPrintMenuAbout:
        //            Slic3r::GUI::about();
        //            break;
        //        case CrealityPrintMenuPreferences:
        //            CallAfter([this] {
        //                PreferencesDialog dlg(this);
        //                dlg.ShowModal();
        //#if ENABLE_GCODE_LINES_ID_IN_H_SLIDER
    ...
    append_menu_item(calibMenu, wxID_ANY, _L("Temperature"), _L("Temperature Calibration"),
        [this](wxCommandEvent&) {
            Temp_Calibration_Dlg((wxWindow*)this, wxID_ANY, m_plater).ShowModal();
        }, "", nullptr,
    ...
    append_menu_item(
        ...
        [this](wxCommandEvent&) {
            if (!m_high_flowrate_dlg)
                m_high_flowrate_dlg = new High_Flowrate_Dlg((wxWindow*) this, wxID_ANY, m_plater);
            m_high_flowrate_dlg->ShowModal();
        },
        "", nullptr,
        [this]() {
        ...
        this);
    //calibMenu->AppendSubMenu(flowrate_menu, _L("Flow rate"));
    append_submenu(
        calibMenu, flowrate_menu, wxID_ANY, _L("Flow rate"), _L("Flow rate"), "",
        [this]() {
            return m_plater->is_view3D_shown() && m_tabpanel->GetSelection() == TabPosition::tp3DEditor;
            ;
        },
    ...
    append_menu_item(calibMenu, wxID_ANY, _L("Pressure advance"), _L("Pressure advance"),
        [this](wxCommandEvent&) {
            if (!m_pa_calib_dlg)
    ...
    append_menu_item(
        retraction_test, wxID_ANY, _L("Retraction distance test"), _L("Retraction distance test"),
        [this](wxCommandEvent&) {
        ...
        this);
    append_menu_item(
        retraction_test, wxID_ANY, _L("Retraction speed test"), _L("Retraction speed test"),
        [this](wxCommandEvent&) {
            if (!m_retraction_speed_calib_dlg)
                m_retraction_speed_calib_dlg = new Retraction_Speed_Dlg((wxWindow*) this, wxID_ANY, m_plater);
            m_retraction_speed_calib_dlg->ShowModal();
        },
        "", nullptr,
        [this]() {
            return m_plater->is_view3D_shown() && m_tabpanel->GetSelection() == TabPosition::tp3DEditor;
        ...
        this);
    //calibMenu->AppendSubMenu(retraction_test, _L("Retraction test"));
    append_submenu(
        calibMenu, retraction_test, wxID_ANY, _L("Retraction test"), _L("Retraction test"), "",
        [this]() {
            return m_plater->is_view3D_shown() && m_tabpanel->GetSelection() == TabPosition::tp3DEditor;
            ;
        },
    ...
    append_menu_item(calibMenu, wxID_ANY, _L("Tolerance Test"), _L("Tolerance Test"),
        [this](wxCommandEvent&) {
            {
                const auto _name = wxString::Format(L"Tolerance Test");
                if (m_plater->new_project(false, false, _name) == wxID_CANCEL)
                    return;
            }
        m_plater->add_model(false, Slic3r::resources_dir() + "/calib/tolerance_test/CrealityToleranceTest.stl");
        }, "", nullptr,
        [this]() {return m_plater->is_view3D_shown()&& m_tabpanel->GetSelection() == TabPosition::tp3DEditor;; }, this);

    append_menu_item(
        calibMenu, wxID_ANY, _L("Max flowrate"), _L("Max flowrate"),
        [this](wxCommandEvent&) {
            if (!m_vol_test_dlg)
    ...
    int mode = Slic3r::GUI::wxGetApp().get_mode();
    m_topbar->update_mode(mode);
    ...
    if(calibMenu)
        m_menubar->Append(calibMenu, wxString::Format("&%s", _L("Calibration")));
    if (helpMenu)
...
class ConfigsOverwriteConfirmNewDialog : public DPIDialog
{
public:
    ConfigsOverwriteConfirmNewDialog(wxWindow* parent, wxString name)
        : DPIDialog(parent ? parent : nullptr, wxID_ANY, _L("Conflict"), wxDefaultPosition, wxDefaultSize, wxCAPTION | wxCLOSE_BOX)
    {
        this->SetBackgroundColour(*wxWHITE);
        wxGetApp().UpdateDlgDarkUI(this);
        this->SetMinSize(wxSize(FromDIP(587), FromDIP(260)));
        wxBoxSizer* m_main_sizer = new wxBoxSizer(wxVERTICAL);

        //  add top
        wxBoxSizer* pSttSizer = new wxBoxSizer(wxHORIZONTAL);
        pSttSizer->SetMinSize(wxSize(FromDIP(537), FromDIP(27)));

        wxStaticText* pSttText = new wxStaticText(this, wxID_ANY, _L("The following preset files with the same name exist"),
                                                  wxDefaultPosition, wxDefaultSize, wxALIGN_LEFT);
        pSttText->SetForegroundColour(wxColour("#FFFFFF"));
        pSttText->SetFont(Label::Body_12);
        pSttText->SetSize(wxSize(FromDIP(537), FromDIP(27)));
        pSttText->SetMinSize(wxSize(FromDIP(537), FromDIP(27)));
        pSttSizer->Add(pSttText, 0, wxLeft | wxALIGN_CENTER_VERTICAL, FromDIP(0));
        pSttSizer->Layout();
        m_main_sizer->Add(pSttSizer, 0, wxEXPAND | wxALL | wxALIGN_CENTER_VERTICAL, FromDIP(24));

        //  mid panel
        wxPanel* panel = new wxPanel(this, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxTAB_TRAVERSAL);
        panel->SetMinSize(wxSize(FromDIP(537), FromDIP(176)));
        panel->SetBackgroundColour(wxColour("#36363A"));
        m_main_sizer->Add(panel, 1, wxEXPAND | wxALL, 12);

        wxBoxSizer* panel_sizer = new wxBoxSizer(wxVERTICAL);
        panel->SetSizer(panel_sizer);

        pSttSizer = new wxBoxSizer(wxHORIZONTAL);
        pSttSizer->SetMinSize(wxSize(FromDIP(537), FromDIP(24)));

        wxString ssTip = wxString::Format(_L("A file exists with the same name: %s, do you want to override it."), name);
        pSttText       = new wxStaticText(panel, wxID_ANY, ssTip, wxDefaultPosition, wxDefaultSize, wxALIGN_LEFT);
        pSttText->SetForegroundColour(wxColour("#FFFFFF"));
        pSttText->SetFont(Label::Body_12);
        pSttText->SetSize(wxSize(FromDIP(537), FromDIP(24)));
        pSttText->SetMinSize(wxSize(FromDIP(537), FromDIP(24)));
        pSttSizer->Add(pSttText, 0, wxLeft | wxALIGN_CENTER_VERTICAL, FromDIP(0));
        pSttSizer->Layout();
        panel_sizer->Add(pSttSizer, 0, wxEXPAND | wxALL | wxALIGN_CENTER_VERTICAL, FromDIP(8));

        //  add bottom button
        wxBoxSizer* pBottomBox = new wxBoxSizer(wxHORIZONTAL);
        pBottomBox->SetMinSize(wxSize(FromDIP(537), FromDIP(42)));
        pBottomBox->Add(0, 0, 1, wxEXPAND, 5);
        wxButton* pBtnCreateCopy = new wxButton(this, wxID_ANY, _L("Create Copy"), wxDefaultPosition, wxDefaultSize, wxLeft | wxBORDER_NONE);
        pBtnCreateCopy->SetFont(Label::Body_12);
        ...
        pBottomBox->Add(pBtnCreateCopy, 0, wxEXPAND | wxALL, FromDIP(5));
        wxButton* pBtnOverride = new wxButton(this, wxID_ANY, _L("Override"), wxDefaultPosition, wxDefaultSize, wxLeft | wxBORDER_NONE);
        pBtnOverride->SetFont(Label::Body_12);
        pBtnOverride->SetSize(wxSize(FromDIP(128), FromDIP(32)));
        pBtnOverride->SetMinSize(wxSize(FromDIP(128), FromDIP(32)));
        pBtnOverride->SetForegroundColour(wxColour("#FFFFFF"));
        pBtnOverride->SetBackgroundColour(wxColour("#6E6E72"));
        pBtnOverride->Bind(wxEVT_LEFT_DOWN, [this](wxMouseEvent& e) {
            m_nClickedButtonValue = 1;
            EndModal(wxID_OK);
        });
        pBottomBox->Add(pBtnOverride, 0, wxEXPAND | wxALL, FromDIP(5));
        pBottomBox->Add(0, 0, 1, wxEXPAND, FromDIP(5));
        m_main_sizer->Add(pBottomBox);
    ...
    ConfigsOverwriteConfirmNewDialog(wxWindow* parent)
        : DPIDialog(parent ? parent : nullptr, wxID_ANY, _L("Import exception"), wxDefaultPosition, wxDefaultSize, wxCAPTION | wxCLOSE_BOX)
    {
        this->SetBackgroundColour(*wxWHITE);
        wxGetApp().UpdateDlgDarkUI(this);
        this->SetMinSize(wxSize(FromDIP(600), FromDIP(400)));
        wxBoxSizer* m_main_sizer = new wxBoxSizer(wxVERTICAL);
        ...
        wxStaticText* pSttText = new wxStaticText(this, wxID_ANY, _L("There are preset files with the same name or other issues"),
                                                  wxDefaultPosition, wxDefaultSize, wxALIGN_CENTRE);
        pSttText->SetForegroundColour(wxColour("#FFFFFF"));
        //pSttText->SetBackgroundColour(wxColour("#FF0000"));
        pSttText->SetFont(Label::Body_12);
        pSttText->SetSize(wxSize(FromDIP(537), FromDIP(14)));
        pSttText->SetMinSize(wxSize(FromDIP(537), FromDIP(14)));
        pSttSizer->Add(pSttText, 0, wxEXPAND|wxALL, FromDIP(10));
        pSttSizer->Layout();
        m_main_sizer->Add(pSttSizer, 0, wxEXPAND|wxLEFT|wxRIGHT, FromDIP(21));
        ...
        //wxBoxSizer* panel_sizer = new wxBoxSizer(wxVERTICAL);
        //m_panel_sizer           = panel_sizer;
        //panel->SetSizer(panel_sizer);

        wxPanel* panel = new wxPanel(this, wxID_ANY, wxDefaultPosition, wxDefaultSize,
                                     wxTAB_TRAVERSAL | wxBORDER_SIMPLE | wxBG_STYLE_COLOUR);

        panel->SetMinSize(wxSize(FromDIP(558), FromDIP(28)));
        panel->SetBackgroundColour(wxColour("#5B5B5E"));
        wxBoxSizer* panelBoxSizer = new wxBoxSizer(wxHORIZONTAL);
        wxStaticText* sttFile       = new wxStaticText(panel, wxID_ANY, "", wxDefaultPosition, wxDefaultSize, wxALIGN_LEFT);
        m_sttFile                   = sttFile;
        sttFile->SetMinSize(wxSize(FromDIP(558), FromDIP(26)));
        sttFile->SetFont(Label::Body_12);
        sttFile->SetForegroundColour(wxColour("#FFFFFF"));
        panelBoxSizer->Add(sttFile, 0, wxLEFT|wxTOP, FromDIP(8));
        panel->SetSizer(panelBoxSizer);
        m_main_sizer->Add(panel, 0, wxLEFT, FromDIP(21));

        panel = new wxPanel(this, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxTAB_TRAVERSAL | wxBORDER_SIMPLE | wxBG_STYLE_COLOUR);
        panel->SetMinSize(wxSize(FromDIP(558), FromDIP(232)));
        panel->SetBackgroundColour(wxColour("#5B5B5E"));
        panelBoxSizer  = new wxBoxSizer(wxHORIZONTAL);
        wxScrolledWindow* scrolledWindow = new wxScrolledWindow(panel, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxVSCROLL|wxBORDER_SIMPLE);
        m_scrolledWindow                 = scrolledWindow;
        scrolledWindow->SetMinSize(wxSize(FromDIP(558), FromDIP(232)));
        ...
        pBottomBox->Add(pBtnOverride, 0, wxEXPAND | wxALL, FromDIP(5));
        pBottomBox->Add(0, 0, 1, wxEXPAND, FromDIP(5));
        m_main_sizer->Add(pBottomBox, 0, wxTOP, FromDIP(15));
    ...
    void showMiltiFile(const std::list<PresetBundle::STOverrideConfirmFile*>& lstOverrideConfim) {
        for (auto iter = lstOverrideConfim.begin(); iter != lstOverrideConfim.end(); iter++) {
            if (iter != lstOverrideConfim.begin())
                break;
            PresetBundle::STOverrideConfirmFile* pConfirmFile = *iter;
            m_sttFile->SetLabelText(_L("File") + ":" + from_u8(pConfirmFile->fileName));
            int           index = 0;
            for (auto item : pConfirmFile->lstPrinterPreset) {
                wxBoxSizer*   boxSizer = new wxBoxSizer(wxHORIZONTAL);
                wxPanel*    panel    = nullptr;
                if (index++ == 0)
                    panel = createLineData(m_scrolledWindow, _L("Printer with the same name:"), from_u8(item));
                else
                    panel = createLineData(m_scrolledWindow, "", from_u8(item));
                boxSizer->Add(panel);
                m_scrolledWindowSizer->Add(boxSizer);
            }
            if (pConfirmFile->lstPrinterPreset.size() == 0) {
                wxBoxSizer* boxSizer = new wxBoxSizer(wxHORIZONTAL);
                wxPanel*    panel    = createLineData(m_scrolledWindow, _L("Printer with the same name:"), _L("None"));
                boxSizer->Add(panel);
                m_scrolledWindowSizer->Add(boxSizer);
            }

            index = 0;
            for (auto item : pConfirmFile->lstFilamentPreset) {
                wxBoxSizer* boxSizer = new wxBoxSizer(wxHORIZONTAL);
                wxPanel*    panel    = nullptr;
                if (index++ == 0)
                    panel = createLineData(m_scrolledWindow, _L("Filament with the same name:"), from_u8(item));
                else
                    panel = addLineData(m_scrolledWindow, "", from_u8(item));
                boxSizer->Add(panel);
                m_scrolledWindowSizer->Add(boxSizer);
            }
            if (pConfirmFile->lstFilamentPreset.size() == 0) {
                wxBoxSizer* boxSizer = new wxBoxSizer(wxHORIZONTAL);
                wxPanel*    panel    = createLineData(m_scrolledWindow, _L("Filament with the same name:"), _L("None"));
                boxSizer->Add(panel);
                m_scrolledWindowSizer->Add(boxSizer);
            }

            index = 0;
            for (auto item : pConfirmFile->lstProcessPreset) {
                wxBoxSizer* boxSizer = new wxBoxSizer(wxHORIZONTAL);
                wxPanel*    panel    = nullptr;
                if (index++ == 0)
                    panel = createLineData(m_scrolledWindow, _L("Process with the same name:"), from_u8(item));
                else
                    panel = addLineData(m_scrolledWindow, "", from_u8(item));
                boxSizer->Add(panel);
                m_scrolledWindowSizer->Add(boxSizer);
            }
            if (pConfirmFile->lstProcessPreset.size() == 0) {
                wxBoxSizer* boxSizer = new wxBoxSizer(wxHORIZONTAL);
                wxPanel*    panel    = createLineData(m_scrolledWindow, _L("Process with the same name:"), _L("None"));
                boxSizer->Add(panel);
                m_scrolledWindowSizer->Add(boxSizer);
            }
        }
...
private:
    wxPanel* createLineData(wxWindow* parent, wxString title, wxString fileName) {
        ...
        panel->SetMinSize(wxSize(FromDIP(536), FromDIP(26)));
        panel->SetBackgroundColour(wxColour("#4B4B4D"));
        wxBoxSizer*   panelBoxSizer = new wxBoxSizer(wxHORIZONTAL);
        wxStaticText* sttTitle      = new wxStaticText(panel, wxID_ANY, title, wxDefaultPosition, wxDefaultSize, wxALIGN_LEFT);
        sttTitle->SetMinSize(wxSize(FromDIP(100), FromDIP(14)));
        sttTitle->SetFont(Label::Body_12);
        sttTitle->SetForegroundColour(wxColour("#FD265A"));
        panelBoxSizer->Add(sttTitle, 0, wxLEFT | wxTOP, FromDIP(8));
        wxStaticText* sttFile = new wxStaticText(panel, wxID_ANY, fileName, wxDefaultPosition, wxDefaultSize, wxALIGN_LEFT);
        sttFile->SetMinSize(wxSize(FromDIP(280), FromDIP(14)));
        sttFile->SetFont(Label::Body_12);
        sttFile->SetForegroundColour(wxColour("#FFFFFF"));
        panelBoxSizer->Add(sttFile, 0, wxLEFT | wxTOP, FromDIP(8));
        panel->SetSizer(panelBoxSizer);
    ...
    wxPanel* addLineData(wxWindow* parent, wxString title, wxString fileName)
    {
        wxPanel* panel = new wxPanel(parent, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxTAB_TRAVERSAL | wxBG_STYLE_COLOUR);

        panel->SetMinSize(wxSize(FromDIP(536), FromDIP(16)));
        panel->SetBackgroundColour(wxColour("#4B4B4D"));
        wxBoxSizer*   panelBoxSizer = new wxBoxSizer(wxHORIZONTAL);
        wxStaticText* sttTitle      = new wxStaticText(panel, wxID_ANY, title, wxDefaultPosition, wxDefaultSize, wxALIGN_LEFT);
        sttTitle->SetMinSize(wxSize(FromDIP(100), FromDIP(14)));
        sttTitle->SetFont(Label::Body_12);
        sttTitle->SetForegroundColour(wxColour("#FD265A"));
        panelBoxSizer->Add(sttTitle, 0, wxLEFT, FromDIP(8));
        wxStaticText* sttFile = new wxStaticText(panel, wxID_ANY, fileName, wxDefaultPosition, wxDefaultSize, wxALIGN_LEFT);
        sttFile->SetMinSize(wxSize(FromDIP(280), FromDIP(14)));
        sttFile->SetFont(Label::Body_12);
        sttFile->SetForegroundColour(wxColour("#FFFFFF"));
        panelBoxSizer->Add(sttFile, 0, wxLEFT, FromDIP(8));
        panel->SetSizer(panelBoxSizer);
...
private:
    void on_dpi_changed(const wxRect& suggested_rect) override { Layout(); }
    int  m_nClickedButtonValue = 0;
    wxStaticText*     m_sttFile                  = nullptr;
    wxScrolledWindow* m_scrolledWindow          = nullptr;
    wxBoxSizer*       m_scrolledWindowSizer      = nullptr;
};

void MainFrame::export_config(const wxString& export_type)
{
...
void MainFrame::export_presets()
{
    ExportConfigsDialog* pExport_configs_dlg  = new ExportConfigsDialog(nullptr, 1);
    pExport_configs_dlg->exportPresets();
    delete pExport_configs_dlg;
}
...
void MainFrame::load_config_file()
{
 ...
 //       return;
    wxFileDialog dlg(this, _L("Select profile to load:"),
        ...
        "config.json", "Config files (*.json;*.zip;*.creality_printer;*.creality_filament)|*.json;*.zip;*.creality_printer;*.creality_filament", wxFD_OPEN | wxFD_MULTIPLE | wxFD_FILE_MUST_EXIST);
     wxArrayString files;
    if (dlg.ShowModal() != wxID_OK)
        return;
    dlg.GetPaths(files);
    ...
    std::vector<std::string>        vtNewDataFile;
    std::shared_ptr<LoadOldPresets> spLoadOldPresets = std::make_shared<LoadOldPresets>();
    spLoadOldPresets->setOverrideConfirmCb([this](const std::list<PresetBundle::STOverrideConfirmFile*>& lstOverrideConfirm) -> int {
        ConfigsOverwriteConfirmNewDialog dlg(nullptr);
    ...
    /*wxGetApp().preset_bundle->import_presets(
        ...
        [this](std::string const& name) {
            ConfigsOverwriteConfirmNewDialog dlg(nullptr, from_u8(name));
            dlg.ShowModal();
            return dlg.getClickedButtonValue();
            /*
    ...
    wxGetApp().preset_bundle->import_presets_multifile(
        vtNewDataFile,
        [this](const std::list<PresetBundle::STOverrideConfirmFile*>& lstOverrideConfirm) -> int {
            ConfigsOverwriteConfirmNewDialog dlg(nullptr);
            dlg.showMiltiFile(lstOverrideConfirm);
            return dlg.getClickedButtonValue();
        },
        ForwardCompatibilitySubstitutionRule::Enable);
    if (!vtNewDataFile.empty()) {
        wxGetApp().app_config->update_config_dir(get_dir_name(vtNewDataFile.back()));
        wxGetApp().load_current_presets();
        BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << " presets has been import,and size is" << vtNewDataFile.size();
        NetworkAgent* agent = wxGetApp().getAgent();
        if (agent) {
...
void MainFrame::loadOldPresetsFromFolder()
{
    std::string userDataDir = wxStandardPaths::Get().GetUserDataDir().ToUTF8().data();
    std::string  oldPresetsFolder           = userDataDir + "/Creative3D/5.0/server_0/orca/user/parampack";
    std::shared_ptr<LoadOldPresetsFromFolder> spLoadOldPresetsFromFolder = std::make_shared<LoadOldPresetsFromFolder>();
    spLoadOldPresetsFromFolder->setOverrideConfirmCb(
        [this](const std::list<PresetBundle::STOverrideConfirmFile*>& lstOverrideConfirm) -> int {
        ConfigsOverwriteConfirmNewDialog dlg(nullptr);
        dlg.showMiltiFile(lstOverrideConfirm);
        return dlg.getClickedButtonValue();
    ...
    dlg2.ShowModal();
    wxGetApp().app_config->set("hasImoport5xPreset", "1");
}

void MainFrame::checkHaveOldPresetsNeedLoad()
{
    std::string hasImoport5xPreset = wxGetApp().app_config->get("hasImoport5xPreset");
    BOOST_LOG_TRIVIAL(warning) << __FUNCTION__ << " hasImoport5xPreset=" << hasImoport5xPreset;
    if (hasImoport5xPreset == "1")
        return;
    std::string userDataDir      = wxStandardPaths::Get().GetUserDataDir().ToUTF8().data();
    std::string oldPresetsFolder = userDataDir + "/Creative3D/5.0/server_0/orca/user/parampack";
    if (!LoadOldPresetsFromFolder::hasOldPresets(oldPresetsFolder))
        return;

    wxString      msg(_L("Do you want to migrate user presets from version 5.x?"));
    MessageDialog dlg(this, msg, _L(""), wxYES|wxNO);
...
void MainFrame::open_recent_project(size_t file_id, wxString const & filename)
{
    ...
    {
        MessageDialog msg(this, _L("The project is no longer available. Do you want to delete it from the history list?"), _L("Error"),
