...
public:
    ...
    void init_tabpanel();
    void slice_plate(SliceSelectType all);
    void print_plate(PrintSelectType);
    ...
    void        reslice_now();
    void export_config(const wxString& export_type = "");
    void        export_presets();
    // Query user for the config file and open it.
    void        load_config_file();
    void        loadOldPresetsFromFolder();
    void        checkHaveOldPresetsNeedLoad();
    ...
    //Creality: control the tab event
    void        enable_tab_event(bool enabled) { m_tab_event_enabled = enabled; }   // enable or disable the tab switch event
    ...
    Limit_Acceleration_Dlg*      m_limit_acc_dlg{nullptr};
